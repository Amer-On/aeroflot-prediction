...
- react-toastify - для отправки сообщений пользователю
...
npm run build
```
Разрешение на исполнение скрипта
Вместо того, чтобы прописывать ```npm run build``` (для запуска по аналогии с backend'ом)
```
chmod 777 run.sh
. run.sh
```
